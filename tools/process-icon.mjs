// 一次性工具：神像图标去黑底（边缘泛洪，封边后只移除外部黑背景）
import fs from 'node:fs';
import zlib from 'node:zlib';
const SRC = process.argv[2] || 'E:/demo2/图标/神像.png';
const DST = process.argv[3] || 'e:/demo2/public/idol.png';
...
const channels = colorType === 6 ? 4 : 3, bpp = channels, stride = width * bpp;
...
for (let y = 0; y < height; y++) {
  ...
  for (let x = 0; x < stride; x++) {
    const a = x >= bpp ? px[y * stride + x - bpp] : 0, b = prev[x], c = x >= bpp ? prev[x - bpp] : 0;
    let v = line[x];
    if (f === 1) v = (v + a) & 255;
...
const out = channels === 4 ? px : (() => {
  const rgba = Buffer.alloc(width * height * 4);
  for (let i = 0; i < width * height; i++) { rgba[i * 4] = px[i * 3]; rgba[i * 4 + 1] = px[i * 3 + 1]; rgba[i * 4 + 2] = px[i * 3 + 2]; rgba[i * 4 + 3] = 255; }
  return rgba;
})();
const isBlack = i => { const o = i * 4; return out[o] < 46 && out[o + 1] < 46 && out[o + 2] < 46; };
...
for (let r = 0; r < 4; r++) {
  ...
  for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
    ...
    if ((x > 0 && content[i - 1]) || (x < width - 1 && content[i + 1]) || (y > 0 && content[i - width]) || (y < height - 1 && content[i + width])) next[i] = 1;
  }
  content = next;
}
...
for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) if (out[(y * width + x) * 4 + 3] > 0) {
  if (x < minX) minX = x; if (x > maxX) maxX = x; if (y < minY) minY = y; if (y > maxY) maxY = y;
}
const cw = maxX - minX + 1, ch = maxY - minY + 1;
const crop = Buffer.alloc(cw * ch * 4);
for (let y = 0; y < ch; y++) out.copy(crop, y * cw * 4, ((minY + y) * width + minX) * 4, ((minY + y) * width + minX + cw) * 4);
const CRC = [];
...
const scan = Buffer.alloc(ch * (cw * 4 + 1));
for (let y = 0; y < ch; y++) { scan[y * (cw * 4 + 1)] = 0; crop.copy(scan, y * (cw * 4 + 1) + 1, y * cw * 4, (y + 1) * cw * 4); }
...
console.log(`done ${cw}x${ch} bg removed ${(removed / (width * height) * 100).toFixed(1)}% -> ${DST}`);
