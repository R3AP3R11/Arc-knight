...
| `src/systems/ui/hud.js` | 战斗 HUD 状态机（explore/combat/secure/black 脉冲）与 HUD 各元素绘制；`drawHud` 调 `_trackHudGhosts` 维护血/盾受伤残弧状态（hp/shield 下降记 from→to，约 1s 渐隐） | `HudMixin`（11 个方法）、`HUD_IDLE_MS` | 255 |
...
| `src/systems/ui/world-render.js` | 世界层：背景图、6 类精灵 Map 同步、主渲染循环 `draw()`（含掉落物绘制分派 `:550`） | `WorldRenderMixin`（12 个方法） | 692 |
...
| `src/systems/ui/minimap.js` | **多箱庭小地图**（左上角）：等大箱庭网格 + 每格标记（战斗/神像/宝箱/商人/BOSS）+ 玩家白点。仅 `roomLayout.mode==='multi'` 且非编辑态/非菜单/非结算时绘制；格子统一 `MM_CELL`（不体现箱庭大小差异）；玩家按「所属箱庭内相对位置」映射；标记图标走画板动态资产（`getDesign`+`renderAsset`，空则中文文字占位）。在 `ui-runtime.js` 战斗态兜底分支调用 | `MinimapMixin`（`drawMinimap` / `hideMinimapOverlay`） | 144 |
